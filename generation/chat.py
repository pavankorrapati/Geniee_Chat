...
def _load_instruction_answers() -> list[tuple[str, str, set[str]]]:
    answers: list[tuple[str, str, set[str]]] = []
...
def _load_local_documents() -> list[tuple[str, str]]:
    documents: list[tuple[str, str]] = []

    if CORPUS_DIR.exists():
        documents.extend(load_corpus_documents(CORPUS_DIR))

    # Previously downloaded/cleaned web documents are LOCAL retrieval data,
    # not live internet requests.
    if WEB_CORPUS_DIR.exists():
        documents.extend(
            (f"downloaded_web/{name}", text)
...
class GenieeChat:
    ...
    Those fields are consumed by the FastAPI layer and the UI.
    """
    ...
    def __init__(
        ...
        self.retriever = (
            ...
            else LocalRetriever(_load_local_documents())
        )
    ...
    def context_limit(self) -> int:
        return int(self.generator.model.config.max_seq_len)

    def restore_messages(self, messages: list[dict]) -> None:
        """
        Restore user/assistant messages persisted by ChatHistoryStore.

        ConversationMemory itself remains bounded to its configured max_turns.
        """
    ...
    def _usage_for_direct_answer(self, answer: str) -> None:
        self._set_usage(
            self._count_tokens(self.conversation.prompt()),
            self._count_tokens(answer),
        )

    def _retrieve_answer(self, user_text: str) -> str | None:
        normalized_query = _normalize_question(user_text)
        ...
        exact = next(
            (
                answer
                for question, answer, _ in self.knowledge_base
        ...
        # Conservative semantic-ish lexical match.
        best_answer = None
        best_score = 0.0
    ...
    def _evaluate_local_relevance(
        ...
        reference_terms = _question_terms(
            " ".join(text for _, text in references)
        )
        return len(query_terms & reference_terms) / len(query_terms)

    def _requires_external_context(self, user_text: str) -> bool:
        tokens = set(re.findall(r"[a-z0-9]+", user_text.lower()))
        return bool(tokens & _DYNAMIC_TERMS)

    @staticmethod
    def _format_answer(text: str) -> str:
        """
        Keep useful whitespace/newlines. Avoid the previous implementation
        that collapsed the complete response into one very long line.
        """
        text = text.replace("\r\n", "\n").replace("\r", "\n").strip()
        text = re.sub(r"[ \t]+", " ", text)
        text = re.sub(r"\n{3,}", "\n\n", text)
    ...
    def clean_response(text: str) -> str:
        ...
        return GenieeChat._format_answer(text)
    ...
    def _looks_unreliable(
        ...
        # Common sign of corrupted tiny-model generation.
        ...
        return bool(question_terms) and not bool(
            question_terms & response_terms
        )
    ...
    def _build_prompt(
        ...
        # Keep reference injection deliberately small because the current
        # Geniee checkpoint has a 256-token context window.
        blocks = []
        for name, text in references[:2]:
            cleaned = " ".join(text.split())
            blocks.append(f"[{name}] {cleaned[:500]}")

        reference_text = (
            "\n\nRelevant trusted local reference:\n"
            + "\n".join(blocks)
            + "\n\nUse the reference only when it answers the user's question."
        )
    ...
    def _fit_prompt_to_context(
        ...
        """
        Fit conversation + retrieval context + requested generation inside the
        actual model context window.

        Old turns are removed first. If retrieval itself is still too large,
        references are reduced before generation length is reduced.
        """
        desired_new_tokens = max(1, int(max_new_tokens))
        desired_new_tokens = min(
            desired_new_tokens,
            max(1, self.context_limit - 8),
        )

        working_refs = list(references or [])

        while True:
            prompt = self._build_prompt(working_refs)
            prompt_tokens = self._count_tokens(prompt)

            if prompt_tokens + desired_new_tokens <= self.context_limit:
                return prompt, prompt_tokens, desired_new_tokens
            ...
            if working_refs:
                ...
                if len(text) > 180:
                    working_refs[0] = (name, text[: max(180, len(text) // 2)])
                    continue
                working_refs.clear()
                continue

            # Final fallback: keep at least one output token.
            available = self.context_limit - prompt_tokens
            if available <= 0:
                # The latest user turn + system prompt alone exceeded context.
                # Rebuild from the latest user turn only.
                latest_user = next(
                    (
                        msg["content"]
                ...
                self.conversation.clear()
                if latest_user:
                    self.conversation.add_user(latest_user)
                prompt = self._build_prompt()
                prompt_tokens = self._count_tokens(prompt)
                available = max(1, self.context_limit - prompt_tokens)

            return (
                prompt,
                prompt_tokens,
                max(1, min(desired_new_tokens, available)),
            )
    ...
    def _web_answer(
        ...
        The 27M model is currently too small to reliably summarize arbitrary
        live pages, so web results are returned extractively rather than asking
        the model to rewrite them. This prevents garbled web answers.
        """
        try:
            evidence = self.web_retriever.retrieve(user_text)
        except Exception:
            evidence = []
        ...
        for sentence, url in evidence:
            sentence = self._format_answer(sentence)
            key = sentence.casefold()

            if sentence and key not in seen_snippets:
    ...
    def respond(
        ...
        if traceback_answer:
            traceback_answer = self._format_answer(traceback_answer)
            self.last_source = "debugger"
            self._usage_for_direct_answer(traceback_answer)
        ...
        # ------------------------------------------------------------
        trained_answer = self._retrieve_answer(user_text)
        if trained_answer:
            trained_answer = self._format_answer(trained_answer)
            self.last_source = "trained_qa"
            self._usage_for_direct_answer(trained_answer)
            self.conversation.add_assistant(trained_answer)
            return trained_answer

        # ------------------------------------------------------------
        # 3. Local corpus retrieval
        # ------------------------------------------------------------
        references = self.retriever.retrieve(user_text, limit=2)
        local_relevance = self._evaluate_local_relevance(
            user_text,
            references,
        )

        # Only inject local references when enough of the question is covered.
        useful_references = references if local_relevance >= 0.45 else []

        # ------------------------------------------------------------
        # 4. Live web retrieval
        # ------------------------------------------------------------
        needs_web = (
            self._requires_external_context(user_text)
            or not useful_references
        )

        if enable_web_search and needs_web:
            web_answer, urls = self._web_answer(user_text)

            if web_answer:
                self.last_source = "web"
                self.last_sources = urls
                self._usage_for_direct_answer(web_answer)
                self.conversation.add_assistant(web_answer)
                return web_answer

        # ------------------------------------------------------------
        # 5. Local 27M model with optional local RAG context
        # ------------------------------------------------------------
        if useful_references:
            self.last_source = "local_rag"
            self.last_sources = [
                name for name, _ in useful_references
            ]

        prompt, prompt_tokens, allowed_new_tokens = (
        ...
        # ------------------------------------------------------------
        if self._looks_unreliable(response, user_text):
            if useful_references:
                evidence = self.retriever.best_sentences(
                    user_text,
                    limit=2,
                )
                if evidence:
                    response = self._format_answer(" ".join(evidence))
                    self.last_source = "local_evidence"
                    self.last_sources = [
                        name for name, _ in useful_references
                    ]
                else:
                    response = UNKNOWN_ANSWER
                    self.last_source = "model_fallback"
    ...
    def start(self) -> None:
        ...
        print("Model: Geniee 27M + Local RAG + Web Retrieval")
        ...
        while True:
            ...
            try:
                ...
                print(f"\nGeniee: {response}")
                print(f"Source: {self.last_source}")
...
def load_chatbot() -> GenieeChat:
    ...
    return GenieeChat(
        generator=load_geniee(CHECKPOINT_PATH),
